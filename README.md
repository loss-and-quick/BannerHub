...
If you do not already have any GameHub variant installed, use the **Normal APK** (`banner.hub`). It installs as a completely separate app alongside the official GameHub Lite — both can coexist. If you want BannerHub to take over the `gamehub.lite` package slot (e.g. so apps that launch GameHub Lite by package name launch BannerHub instead), use **Normal.GHL** — but you must **uninstall the official GameHub Lite first** since they share the same package name and have different signing certificates. If you want to use a specific GameHub variant for performance spoofing (e.g. PuBG or Genshin), pick the matching APK — uninstall the existing variant first, then install BannerHub in its place. **Be aware: performance spoofing variants push your device harder and generate significantly more heat. Do not use these variants without proper cooling and a clear understanding of what you are doing. Use at your own risk.**
